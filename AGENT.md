...
| `pnpm run policy` | The three execution modes with what each costs; `-- --set <mode> --yes` writes one to `.env` |
...
pnpm run onboard -- --no-open               # do not open a browser this run
pnpm run onboard -- --open                  # open it again, even if opened here already
