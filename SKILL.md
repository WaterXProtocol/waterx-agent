...
| `not-set-up` | Run `bootstrap` and relay what is still missing. A step reading **"THE ACCOUNT OWNER signs it — YOU run the command below"** means exactly that: their signature, your command. Run the envelope's `nextCommand` — for the grant it is `onboard --wait 300 --json`, which prints the link to hand them and opens the page immediately, and only then waits. Handing over the wallet address instead leaves them with nothing to click. Once it adopts, the policy is still `read-only`: its `nextCommand` is `policy --json`, and the person picks. |
