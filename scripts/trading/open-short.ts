 ...
 * Usage: npx tsx scripts/trading/open-short.ts --base BTC --collateral 10 --leverage 5 [--tp 60000] [--sl 70000]
 */
import { initSigner, requireAccountId, parseArgs, usdcToRaw, fmtTx } from "../lib/init.ts";
import { openShort } from "../../src/agent/index.ts";
...
const args = parseArgs(
  ...
  "scripts/trading/open-short.ts",
);

console.log(`Opening SHORT ${args.base} | ${args.collateral} USDC @ ${args.leverage}x...`);

const result = await openShort(signer, {
  accountId,
...
console.log(`Short opened: ${fmtTx(result.digest)}`);
