 ...
 * ledgers, and it used to be added only alongside `.env`: a project that already
 * ignored `.env` — which a careful person or agent sets up before the key is
 * written — got "already" and no rule for `.waterx/`, so the next `git add .`
 * committed who adopted which account.
 *
